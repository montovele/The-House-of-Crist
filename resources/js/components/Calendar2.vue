<template>
  <div class="calendar">
    ...
    <div class="calendar-grid">
      ...
      <div v-for="date in dates" class="calendar-cell" :class="{ 
  ...
  {{ date.value.date() }}
  <div v-if="menuVisible && !date.isBeforeToday" class="time-menu">
    <div v-for="time in getAvailableTimes(selectedDate)" :key="time" class="time-option" :class="{ 
      'available': isAvailableTime(selectedDate, time), 
      'unavailable': !isAvailableTime(selectedDate, time) 
    }" @click="selectDate(selectedDate, time)">
      {{ time }}
    </div>
  </div>
</div>
...
availableTimes: { /* fill in available times */ },
menuVisible: false,
}
},
computed: {
...
return this.availableTimes[date.format('YYYY-MM-DD')].indexOf(time) !== -1;
},
previousMonth() {
